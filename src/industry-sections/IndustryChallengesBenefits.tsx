...
export function IndustryChallengesBenefits({ content, ctx }: { content: IndustryChallengesBenefitsContent; ctx: SectionCtx }) {
  const { isDarkMode } = ctx;
  const CenterIcon = getIcon(content.centerIcon);

  return (
    <section className={`relative py-20 md:py-28 ${isDarkMode ? 'bg-[#080E1D]' : 'bg-white'}`}>
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        >
          ...
          {/* Benefits panel */}
          <div className={`relative overflow-hidden p-8 sm:p-10 md:p-12 ${isDarkMode ? 'bg-[#0B1220]' : 'bg-white'}`}>
            <div
              aria-hidden
              ...
              style={{
                backgroundImage: isDarkMode
                  ...
                  : 'radial-gradient(circle, rgba(11,31,74,0.10) 1px, transparent 1px)',
            ...
            <div className="relative z-10">
              ...
              <h3 className={`mb-7 text-2xl font-bold leading-tight md:text-[28px] ${isDarkMode ? 'text-white' : 'text-[#0B1F4A]'}`}>{content.benefitsTitle}</h3>
                  ...
                  return (
                    <motion.div
                      ...
                      className={`rounded-xl border p-4 shadow-sm transition-shadow hover:shadow-md ${
                        isDarkMode ? 'border-emerald-400/15 bg-white/[0.04] hover:border-emerald-400/30' : 'border-emerald-100 bg-white'
                      }`}
                    >
                      <span className={`mb-3 flex h-9 w-9 items-center justify-center rounded-full ${isDarkMode ? 'bg-emerald-400/15' : 'bg-emerald-100'}`}>
                        <Icon className="h-4 w-4 text-emerald-600" />
                      </span>
                      <p className={`mb-1 text-sm font-bold leading-snug ${isDarkMode ? 'text-white' : 'text-[#0B1F4A]'}`}>{b.title}</p>
                      <p className={`text-xs leading-relaxed ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}><RichText text={b.description} /></p>
                    </motion.div>
                  );
                })}
              </div>
